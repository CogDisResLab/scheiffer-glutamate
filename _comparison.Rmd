...
# save LFC table
write_csv(differential_phosphorylated_peptides, "results/dpp_{{case}}_{{ctrl}}-{{chip}}.csv")
...
# save file
AvgZTable2 %>% write_csv("results/withinChip_KRSA_Table_{{case}}_{{ctrl}}.csv")
...
pdf(
  "combined_krsa_zscores.pdf",
  width = 8.5,
  height = 11,
  bg = "white"
)
AvgZTable2 |> ungroup() |> filter(abs(AvgZ) >= 2) |> krsa_zscores_plot()
dev.off()
