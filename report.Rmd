---
title: "Glutamate Experiment - Schieffer Lab"
subtitle: "Ziaolu Zhang"
...
params:
  ...
  signal_file: "kinome_data/_ExportstosDBstsCopy_Median_SigmBg_220406124003.txt" # Change the file name to the one you have
  saturation_file: "kinome_data/_ExportstosDBstsCopy_Signal_Saturation_220406124004.txt" # Change the file name to the one you have
...
qc_data <- krsa_qc_steps(raw_data) %>% 
  mutate(
  Group = case_when(
    str_detect(SampleName, "GLU\\d") ~ "GLU",
    str_detect(SampleName, "VEH\\d") ~ "VEH",
    TRUE ~ SampleName
  ),
  SampleName = paste0(Group, "_" , Barcode)
) |> 
  filter(Barcode != "710373302")

groups <- unique(qc_data$Group)

selected_samples <- qc_data |> 
  filter(Group %in% c("GLU", "VEH")) |> 
  pull(SampleName) |> 
  unique()
...
krsa_cv_plot(data_modeled$normalized, clean_peptides, selected_samples)
```
...
krsa_violin_plot(data_modeled$scaled, clean_peptides, "Group", groups = groups[3:4])
...
krsa_violin_plot(data_modeled$scaled, clean_peptides, "Barcode", groups = groups[3:4])
...
krsa_heatmap(data_modeled$normalized, clean_peptides, scale = "row", groups = groups[3:4])
...
krsa_heatmap_grouped(data_modeled$grouped, clean_peptides, scale = "row", groups = groups[3:4])
...
comparisons <- list(COMP1 = c(groups[3], groups[4]))
```
